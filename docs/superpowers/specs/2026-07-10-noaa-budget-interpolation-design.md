...
- **Partition of Unity:** The sum of weights in each non-empty destination row must be $1.0$ (within double-precision epsilon).
- **(Planned) Bilinear Parity:** With `budget_subgrid_size = 1`, the produced `InterpolationMatrix` should match `InterpolationMethod::Bilinear` (up to ordering/tolerance).
- **(Planned) Min Valid Thresholding:** Verification that boundary cells are correctly unmapped or normalized according to `budget_min_valid_fraction`.
