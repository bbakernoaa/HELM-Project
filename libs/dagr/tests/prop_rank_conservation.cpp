        ...
        switch (op_type) {
            case Op_Type::allocate: {
                ...
                if (pool.available_ranks() > 0) {
                    auto count = *rc::gen::inRange<std::uint32_t>(
                        1, pool.available_ranks() + 1);
            ...
            case Op_Type::release: {
                ...
                if (!currently_allocated.empty()) {
                    ...
                    std::vector<int> allocated_vec(currently_allocated.begin(),
                                                   currently_allocated.end());

                    auto release_count = *rc::gen::inRange<std::size_t>(
                        1, allocated_vec.size() + 1);
