...
rc::Gen<std::vector<std::size_t>> gen_fields_to_remove() {
    return rc::gen::exec([] {
        ...
        if (removed.empty()) {
            removed.push_back(
                *rc::gen::inRange<std::size_t>(0, REQUIRED_FIELDS.size()));
        }
        return removed;
    });
}
...
std::string build_yaml_with_missing_fields(
    const std::string& stream_name,
    const std::vector<std::size_t>& removed_indices) {
    ...
    for (std::size_t i = 0; i < REQUIRED_FIELDS.size(); ++i) {
        bool is_removed = std::find(removed_indices.begin(),
                                    removed_indices.end(), i)
                          != removed_indices.end();
        if (!is_removed) {
            yaml += field_values[i];
        }
...
std::filesystem::path write_temp_yaml(const std::string& content) {
    ...
    auto path = std::filesystem::temp_directory_path()
                / ("dagr_prop_missing_field_" + std::to_string(++counter) + ".yaml");
...
RC_GTEST_PROP(MissingFieldRejection,
    ...
    try {
        ...
        RC_FAIL("parse_pipeline did not throw for stream '" + stream_name
                + "' with missing fields: " + removed_desc);
        ...
        std::size_t first_removed = *std::min_element(
