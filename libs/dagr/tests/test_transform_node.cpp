...
enum class MockCall : int { amio_read_left = 0, amio_read_right = 1, amio_view_data_left = 2, amio_view_data_right = 3, blend_dispatch = 4 };
...
amio_status_t amio_read(amio_dataset_handle /*dataset*/, const char * /*var_name*/, int64_t /*timestep*/, const amio_bbox_t * /*bbox*/,
...
amio_status_t amio_view_data(amio_view_handle view, const void **out_data, size_t *out_size) {
...
class TransformNodeTest : public ::testing::Test {
   protected:
    ...
    tick::AliasedWindow make_aliased(double alpha) {
        return tick::AliasedWindow{tick::Time_Window{tick::Time_Point{0}, tick::Time_Point{100}}, alpha};
    }

    /// Invoke dispatch_temporal_bookend with default/configurable parameters.
    void invoke_dispatch(dagr::Temporal_Profile profile = dagr::Temporal_Profile::linear, double alpha = 0.5) {
        ...
        dagr::detail::dispatch_temporal_bookend(stream, aliased, dataset, var_name, t_left, t_right, output, MockState::kFieldSize, token);
    }
};
...
TEST_F(TransformNodeTest, CallSequence_LeftFetchAlwaysPrecedesRightFetch) {
    ...
    EXPECT_LT(left_read_idx, right_read_idx) << "Left bookend fetch must precede right bookend fetch (Req 4.1)";
}
...
TEST_F(TransformNodeTest, CallSequence_BlendDispatchIsLast) {
    ...
    EXPECT_EQ(g_mock.call_sequence.back(), MockCall::blend_dispatch) << "BLEND dispatch must be the final operation (Req 4.3)";
}
...
TEST_F(TransformNodeTest, LinearProfile_RoutesToLinearBlendKernel) {
    ...
    EXPECT_EQ(g_mock.blend_profile_tag, 0) << "Temporal_Profile::linear must route to profile_tag 0 (linear-blend)";
}
...
TEST_F(TransformNodeTest, StepProfile_RoutesToStepSelectKernel) {
    ...
    EXPECT_EQ(g_mock.blend_profile_tag, 1) << "Temporal_Profile::step must route to profile_tag 1 (step-select)";
}
...
TEST_F(TransformNodeTest, AmioReadLeftFailure_PropagatesError) {
    ...
    EXPECT_THROW(invoke_dispatch(), std::runtime_error) << "AMIO left fetch failure must propagate as std::runtime_error (Req 4.8)";

    // BLEND should never be called
    for (const auto &call : g_mock.call_sequence) {
        EXPECT_NE(call, MockCall::blend_dispatch) << "BLEND must not be dispatched when AMIO fetch fails";
    }
}
...
TEST_F(TransformNodeTest, AmioReadRightFailure_PropagatesError) {
    ...
    EXPECT_THROW(invoke_dispatch(), std::runtime_error) << "AMIO right fetch failure must propagate as std::runtime_error (Req 4.8)";
...
TEST_F(TransformNodeTest, AmioViewDataLeftFailure_PropagatesError) {
    ...
    EXPECT_THROW(invoke_dispatch(), std::runtime_error) << "AMIO view_data failure must propagate as std::runtime_error (Req 4.8)";
}

TEST_F(TransformNodeTest, AmioViewDataRightFailure_PropagatesError) {
    g_mock.amio_view_data_right_rc = AMIO_ERR_INVALID_HANDLE;

    EXPECT_THROW(invoke_dispatch(), std::runtime_error) << "AMIO right view_data failure must propagate as std::runtime_error (Req 4.8)";
}
...
TEST_F(TransformNodeTest, AlphaIsPassedThroughWithoutModification) {
    ...
    EXPECT_EQ(g_mock.blend_alpha, test_alpha) << "Alpha must be passed through to BLEND without modification (Req 4.2, 7.3, 7.5)";
}
...
TEST_F(TransformNodeTest, BlendFailure_PropagatesError) {
    ...
    EXPECT_THROW(invoke_dispatch(), std::runtime_error) << "BLEND execution failure must propagate as std::runtime_error (Req 4.10)";
}
...
TEST_F(TransformNodeTest, BlendFailure_AmioCallsStillComplete) {
    ...
    try {
        ...
        EXPECT_EQ(g_mock.amio_read_call_count, 2) << "Both AMIO reads must complete before BLEND dispatch";
        EXPECT_EQ(g_mock.amio_view_data_call_count, 2) << "Both AMIO view_data calls must complete before BLEND dispatch";
    }
}
...
TEST_F(TransformNodeTest, ZeroComputation_FieldDataPointersRoutedDirectly) {
    ...
    EXPECT_EQ(g_mock.blend_left_ptr, g_mock.left_data) << "Left FieldView pointer must be routed directly from AMIO to BLEND (Req 4.5)";
    EXPECT_EQ(g_mock.blend_right_ptr, g_mock.right_data) << "Right FieldView pointer must be routed directly from AMIO to BLEND (Req 4.5)";
}
...
TEST_F(TransformNodeTest, ZeroComputation_OutputBufferUntouchedByDAGR) {
    ...
    dagr::detail::dispatch_temporal_bookend(stream, aliased, dataset, "var", 0, 1, output, MockState::kFieldSize, token);

    // DAGR must not write to the output buffer — only BLEND does (and our mock doesn't)
    EXPECT_EQ(std::memcmp(output, expected, sizeof(output)), 0) << "DAGR must not write to output buffer — only BLEND may (Req 7.4)";
}
...
TEST_F(TransformNodeTest, ZeroComputation_ElementCountPassedThrough) {
    ...
    EXPECT_EQ(g_mock.blend_count, expected_count) << "Element count must be passed through to BLEND without modification";
}
