...
int Value::as_int() const {
    if (!node_)
        throw Conf_Error(Error_Code::Type_Mismatch, "undefined node");
    const auto* n = static_cast<const YAML::Node*>(node_);
    if (!n->IsScalar())
...
std::optional<int> Value::try_int() const noexcept {
    try { return as_int(); } catch (...) { return std::nullopt; }
}

std::optional<double> Value::try_double() const noexcept {
    try { return as_double(); } catch (...) { return std::nullopt; }
}

std::optional<bool> Value::try_bool() const noexcept {
    try { return as_bool(); } catch (...) { return std::nullopt; }
}

std::optional<std::string> Value::try_string() const noexcept {
    try { return as_string(); } catch (...) { return std::nullopt; }
}
