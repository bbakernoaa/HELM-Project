...
int conf_load_c(const char *path, int path_len, int *handle_out) {
    ...
    CONF_C_TRY(std::string p(path, static_cast<std::size_t>(path_len)); auto *cfg = new conf::Config(conf::Config::from_file(p));
               *handle_out = conf::fortran::Handle_Registry::instance().register_handle(static_cast<void *>(cfg));)
}
...
int conf_load_string_c(const char *yaml_text, int text_len, int *handle_out) {
    ...
    CONF_C_TRY(std::string text(yaml_text, static_cast<std::size_t>(text_len)); auto *cfg = new conf::Config(conf::Config::from_string(text));
               *handle_out = conf::fortran::Handle_Registry::instance().register_handle(static_cast<void *>(cfg));)
}
...
int conf_has_key_c(int handle, const char *key, int key_len, int *exists_out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len)); *exists_out = cfg->has(k) ? 1 : 0;)
}
...
int conf_size_c(int handle, const char *key, int key_len, int *size_out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len));
...
int conf_get_int_c(int handle, const char *key, int key_len, int *out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len)); *out = cfg->get_int(k);)
}
...
int conf_get_double_c(int handle, const char *key, int key_len, double *out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len)); *out = cfg->get_double(k);)
}
...
int conf_get_bool_c(int handle, const char *key, int key_len, int *out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len)); *out = cfg->get_bool(k) ? 1 : 0;)
}
...
int conf_get_string_len_c(int handle, const char *key, int key_len, int *str_len_out) {
    ...
    CONF_C_TRY(std::string k(key, static_cast<std::size_t>(key_len)); std::string v = cfg->get_string(k);  // throws Key_Not_Found / Type_Mismatch
...
int conf_get_string_c(int handle, const char *key, int key_len, char *buf, int buf_cap, int *written_out) {
    ...
    CONF_C_TRY(
        std::string k(key, static_cast<std::size_t>(key_len)); std::string v = cfg->get_string(k);  // throws Key_Not_Found / Type_Mismatch
        ...
        if (need > buf_cap) { return static_cast<int>(conf::Error_Code::Buffer_Too_Small); }

        // Copy into the caller buffer. For empty strings (need == 0), this
        // is a no-op. Never null-terminate.
        if (need > 0) { std::memcpy(buf, v.data(), static_cast<std::size_t>(need)); } *written_out = need;)
}
