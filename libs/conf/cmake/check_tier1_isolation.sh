...
#
# Scans all CONF source and header files for #include directives that reference
# forbidden HELM component path segments (halo/, logs/, axis/, amio/, span/,
# dagr/, tick/, blend/), case-insensitive.
#
...
#
# Usage:
#   check_tier1_isolation.sh [CONF_ROOT]
#
# Arguments:
#   CONF_ROOT  Path to the CONF library root (defaults to parent of cmake/).
#
# Exit codes:
...
# Determine CONF root directory.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONF_ROOT="${1:-"${SCRIPT_DIR}/.."}"

# Resolve to absolute path.
CONF_ROOT="$(cd "${CONF_ROOT}" && pwd)"

# Directories to scan.
INCLUDE_DIR="${CONF_ROOT}/include"
SRC_DIR="${CONF_ROOT}/src"
...
if [[ ${#FILES[@]} -eq 0 ]]; then
    echo "TIER1_ISOLATION: No source/header files found to scan in ${CONF_ROOT}"
...
# E.g., halo/, logs/, axis/, amio/, span/, dagr/, tick/, blend/
# Note: bare <span> (without '/') is NOT forbidden — it's the C++20 standard header.
FORBIDDEN_PATTERN='#[[:space:]]*include[[:space:]]*[<"][^>"]*\b(halo|logs|axis|amio|span|dagr|tick|blend)/[^>"]*[>"]'
...
if [[ ${VIOLATION_COUNT} -gt 0 ]]; then
    ...
    echo "─────────────────────────────────────────────────────────────────"
    echo ""
    echo "CONF is a Tier 1 library and must NOT include headers from other"
    echo "HELM components (HALO, LOGS, AXIS, AMIO, SPAN, DAGR, TICK, BLEND)."
