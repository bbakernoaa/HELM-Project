...
struct Gregorian_Calendar {
    ...
    [[nodiscard]] static constexpr std::int32_t days_in_month(std::int32_t year,
                                                              std::int32_t month)
    {
        if (month < 1 || month > 12) {
            throw std::invalid_argument(
                "Invalid month: " + std::to_string(month) + " not in range [1, 12]");
        }
        constexpr std::int32_t table[] = {31, 28, 31, 30, 31, 30,
                                          31, 31, 30, 31, 30, 31};
    ...
    [[nodiscard]] static constexpr Time_Point add_months(Time_Point tp, std::int32_t months)
    {
        ...
        return to_time_point(
            Date_Time{new_year, new_month, new_day, dt.hour, dt.minute, dt.second, dt.nanosecond});
    }
...
private:
    ...
    static constexpr void validate(const Date_Time& dt)
    {
        if (dt.month < 1 || dt.month > 12) {
            throw std::invalid_argument(
                "Invalid month: " + std::to_string(dt.month) + " not in range [1, 12]");
        }

        std::int32_t max_day = days_in_month(dt.year, dt.month);
        if (dt.day < 1 || dt.day > max_day) {
            throw std::invalid_argument(
                "Invalid day: " + std::to_string(dt.day) +
                " not in range [1, " + std::to_string(max_day) + "]");
        }

        if (dt.hour < 0 || dt.hour > 23) {
            throw std::invalid_argument(
                "Invalid hour: " + std::to_string(dt.hour) + " not in range [0, 23]");
        }

        if (dt.minute < 0 || dt.minute > 59) {
            throw std::invalid_argument(
                "Invalid minute: " + std::to_string(dt.minute) + " not in range [0, 59]");
        }

        if (dt.second < 0 || dt.second > 59) {
            throw std::invalid_argument(
                "Invalid second: " + std::to_string(dt.second) + " not in range [0, 59]");
        }

        if (dt.nanosecond < 0 || dt.nanosecond > 999'999'999) {
            throw std::invalid_argument(
                "Invalid nanosecond: " + std::to_string(dt.nanosecond) +
                " not in range [0, 999999999]");
        }
    }
    ...
    [[nodiscard]] static constexpr Civil days_to_civil(std::int64_t z) noexcept
    {
        ...
        return Civil{static_cast<std::int32_t>(yr),
    ...
    [[nodiscard]] static constexpr std::int64_t civil_to_days(std::int32_t y,
                                                              std::int32_t m,
    ...
    [[nodiscard]] static constexpr std::int32_t floor_div_i32(std::int32_t a,
