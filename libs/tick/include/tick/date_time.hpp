...
[[nodiscard]] inline Date_Time parse_iso8601(std::string_view sv) {
    ...
    if (sv.size() != 19) {
        if (sv.size() < 19) {
            throw std::invalid_argument(
                ...
                "got " + std::to_string(sv.size()) + " characters (unexpected end at byte " +
                std::to_string(sv.size()) + ")");
        } else {
            throw std::invalid_argument(
                "ISO 8601 parse error: expected 19 characters \"YYYY-MM-DDThh:mm:ss\", "
                "got " + std::to_string(sv.size()) + " characters (trailing data at byte 19)");
        }
    }
    ...
    auto check_sep = [&](std::size_t pos, char expected) {
        if (sv[pos] != expected) {
            throw std::invalid_argument(
                "ISO 8601 parse error: expected '" + std::string(1, expected) +
                "' at byte " + std::to_string(pos) + ", got '" +
                std::string(1, sv[pos]) + "'");
        }
    };
    check_sep(4, '-');
    ...
    auto parse_int = [&](std::size_t start, std::size_t len) -> std::int32_t {
        ...
        if (ec != std::errc{} || ptr != last) {
            throw std::invalid_argument(
                "ISO 8601 parse error: invalid numeric field at byte " +
                std::to_string(start));
        }
        return value;
    };
    ...
    if (month < 1 || month > 12) {
        throw std::invalid_argument(
            "ISO 8601 parse error: month " + std::to_string(month) +
            " not in range [1, 12]");
    }
    if (day < 1 || day > 31) {
        throw std::invalid_argument(
            "ISO 8601 parse error: day " + std::to_string(day) +
            " not in range [1, 31]");
    }
    if (hour < 0 || hour > 23) {
        throw std::invalid_argument(
            "ISO 8601 parse error: hour " + std::to_string(hour) +
            " not in range [0, 23]");
    }
    if (minute < 0 || minute > 59) {
        throw std::invalid_argument(
            "ISO 8601 parse error: minute " + std::to_string(minute) +
            " not in range [0, 59]");
    }
    if (second < 0 || second > 59) {
        throw std::invalid_argument(
            "ISO 8601 parse error: second " + std::to_string(second) +
            " not in range [0, 59]");
    }
...
[[nodiscard]] inline std::string format_iso8601(const Date_Time& dt) {
    ...
    std::snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02d",
                  ...
                  static_cast<int>(dt.hour),
                  static_cast<int>(dt.minute),
