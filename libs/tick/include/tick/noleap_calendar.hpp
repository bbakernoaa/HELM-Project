...
struct NoLeap_Calendar {
    ...
    [[nodiscard]] static constexpr std::int32_t days_in_month([[maybe_unused]] std::int32_t year,
                                                              std::int32_t month)
    {
        if (month < 1 || month > 12) {
        ...
        constexpr std::int32_t table[12] = {
    ...
    [[nodiscard]] static constexpr Date_Time to_date_time(Time_Point tp)
    {
        ...
        constexpr std::int32_t cum_days[12] = {
    ...
    [[nodiscard]] static constexpr Time_Point to_time_point(Date_Time dt)
    {
        ...
        std::int64_t day_offset  = year_offset * 365
                                 + cum_days[dt.month - 1]
                                 + (dt.day - 1);

        std::int64_t nanos = day_offset * nanos_per_day
    ...
    [[nodiscard]] static constexpr Time_Point add_months(Time_Point tp, std::int32_t months)
    {
        ...
        Date_Time result{target_year, target_month, target_day,
...
private:
    ...
    static constexpr void validate(const Date_Time& dt)
    {
        ...
        if (dt.month == 2 && dt.day == 29) {
            throw std::invalid_argument(
                "Invalid day: February 29 is invalid in the NoLeap calendar");
        }

        if (dt.day < 1 || dt.day > max_day) {
            throw std::invalid_argument(
                "Invalid day: value out of range for the given month");
        }
