...
struct Cal360_Calendar {
    ...
    [[nodiscard]] static constexpr Time_Point to_time_point(Date_Time dt) {
        ...
        std::int64_t day_offset =
            static_cast<std::int64_t>(year_offset) * 360 + static_cast<std::int64_t>(dt.month - 1) * 30 + static_cast<std::int64_t>(dt.day - 1);

        // Sub-day nanoseconds.
        std::int64_t sub_day_nanos = static_cast<std::int64_t>(dt.hour) * nanos_per_hour + static_cast<std::int64_t>(dt.minute) * nanos_per_minute +
                                     static_cast<std::int64_t>(dt.second) * nanos_per_second + static_cast<std::int64_t>(dt.nanosecond);
   ...
   private:
    ...
    static constexpr void validate(const Date_Time &dt) {
        ...
        if (dt.day < 1 || dt.day > 30) {
            throw std::invalid_argument("Invalid day: value not in range [1, 30]");
        }
