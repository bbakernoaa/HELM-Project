...
RC_GTEST_PROP(ContextLabelNormalization,
              EmptyLabelPushesPlaceholder,
              ()) {
...
RC_GTEST_PROP(ContextLabelNormalization,
    ...
    auto raw = *rc::gen::container<std::string>(
        total_len,
...
TEST(ContextStackPerThreadIndependence, ThreadsNeverShareLabels) {
    ...
    auto make_label = [](int thread_idx, int label_idx) -> std::string {
        return "thread_" + std::to_string(thread_idx) + "_label_" +
    ...
    for (int t = 0; t < NUM_THREADS; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (int l = 0; l < LABELS_PER_THREAD; ++l) {
                contexts.push_back(
                    std::make_unique<logs::Scoped_Context>(make_label(t, l)));
            }
    ...
    for (int t = 0; t < NUM_THREADS; ++t) {
        ...
        EXPECT_EQ(snapshot.size(), static_cast<std::size_t>(LABELS_PER_THREAD))
            << "Thread " << t << " has wrong number of labels";

        // Each label in the snapshot must belong to this thread.
        for (int l = 0; l < LABELS_PER_THREAD; ++l) {
            EXPECT_EQ(snapshot[static_cast<std::size_t>(l)], make_label(t, l))
                << "Thread " << t << " has incorrect label at position " << l;
        }
        ...
        for (const auto& label : snapshot) {
            ...
            EXPECT_TRUE(label.find(own_prefix) == 0)
    ...
    EXPECT_EQ(main_after, main_before)
        << "Main thread context stack was modified by child threads";
}

/// Property-based variant: randomize the number of threads and labels per
/// thread, then verify per-thread isolation.
RC_GTEST_PROP(ContextStackPerThreadIndependence,
              RandomizedThreadIsolation,
              ()) {
    ...
    std::vector<std::vector<std::string>> thread_snapshots(
        static_cast<std::size_t>(num_threads));
    std::mutex snapshot_mutex;

    auto make_label = [](int thread_idx, int label_idx) -> std::string {
        return "t" + std::to_string(thread_idx) + "_l" +
               std::to_string(label_idx);
    };
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            {
                std::lock_guard<std::mutex> lock(snapshot_mutex);
                thread_snapshots[static_cast<std::size_t>(t)] =
                    std::move(snapshot);
            }
        });
    }
    ...
    for (int t = 0; t < num_threads; ++t) {
        ...
        for (int l = 0; l < labels_per_thread; ++l) {
            RC_ASSERT(snapshot[static_cast<std::size_t>(l)] ==
                      make_label(t, l));
        }
...
RC_GTEST_PROP(ContextCaptureOrdering,
              EmptyWhenNoContextActive,
              ()) {
...
RC_GTEST_PROP(ContextCaptureOrdering,
              NestedContextsOrderedOutermostToInnermost,
              ()) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto label = *rc::gen::container<std::string>(
            static_cast<std::size_t>(*rc::gen::inRange(1, 65)),
            rc::gen::inRange<char>(33, 127)  // printable non-space ASCII
        );
        RC_PRE(!label.empty());
