...
TEST(EsmfWeightIOTest, NetCdfWeightRoundtrip) {
    ...
    rows(0) = 0; cols(0) = 1; vals(0) = 0.5;
    rows(1) = 0; cols(1) = 2; vals(1) = 0.5;
    rows(2) = 1; cols(2) = 3; vals(2) = 1.0;
    rows(3) = 2; cols(3) = 4; vals(3) = 0.25;
    rows(4) = 2; cols(4) = 5; vals(4) = 0.75;
    rows(5) = 3; cols(5) = 7; vals(5) = 1.0;

    solver::InterpolationMatrix<MemSpace> original_matrix(
        vals, rows, cols,
        Kokkos::View<double*, MemSpace>("frac_a", n_src),
        Kokkos::View<double*, MemSpace>("frac_b", n_dst),
        Kokkos::View<double*, MemSpace>("area_a", n_src),
        Kokkos::View<double*, MemSpace>("area_b", n_dst),
        n_src, n_dst
    );
