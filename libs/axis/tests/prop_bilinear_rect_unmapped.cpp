...
make_nonperiodic_regular_grid(std::size_t ni, std::size_t nj,
...
compute_outside_point(double lon_min, double lon_max,
                      double lat_min, double lat_max,
                      OutsideDirection dir, double offset) {
...
std::size_t
count_row_entries(const axis::solver::InterpolationMatrix<MemSpace>& matrix,
...
RC_GTEST_PROP(PropBilinearRectUnmapped, ErrorPolicyThrows, ()) {
    ...
    auto src_mesh = make_nonperiodic_regular_grid(
        ni, nj, src_lon_min, src_lon_max, src_lat_min, src_lat_max);
    ...
    auto [dst_lon, dst_lat] = compute_outside_point(
        src_lon_min, src_lon_max, src_lat_min, src_lat_max, dir, offset);
