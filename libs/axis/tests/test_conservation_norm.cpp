...
make_mesh(std::size_t n, double size) {
...
TEST(ConservationNorm, DstAreaRawEqualsFracTimesTrue) {
    ...
    for (std::size_t j = 0; j < n_dst; ++j) {
        if (frac_b[j] > 0.0) {
            EXPECT_NEAR(dst_data[j], frac_b[j] * c, 1e-10)
                << "DstArea raw at cell " << j;
        }
    }
...
TEST(ConservationNorm, BothNormsConserve) {
    ...
    for (auto norm : {solver::NormType::DstArea, solver::NormType::FracArea}) {
        ...
        EXPECT_LT(report.relative_error, 1e-12)
            << "Conservation violated for norm type " << static_cast<int>(norm);
    }
}
