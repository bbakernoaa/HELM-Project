...
get_centroids(std::size_t ni, std::size_t nj,
              double lon_start, double lat_start,
              double dlon, double dlat) {
    std::size_t n = ni * nj;
...
RC_GTEST_PROP(PropBilinearExactness, AffineFieldReproduction, ()) {
    ...
    const double a = *rc::gen::map(rc::gen::inRange(-100, 101),
                                   [](int v) { return static_cast<double>(v) / 10.0; });
    const double b = *rc::gen::map(rc::gen::inRange(-100, 101),
                                   [](int v) { return static_cast<double>(v) / 10.0; });
    const double c = *rc::gen::map(rc::gen::inRange(-100, 101),
                                   [](int v) { return static_cast<double>(v) / 10.0; });
