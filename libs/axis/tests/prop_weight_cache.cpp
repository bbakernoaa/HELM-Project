...
RC_GTEST_PROP(PropWeightCache, SerializeDeserializeProducesBitwiseIdenticalApply, ()) {
    ...
    for (std::size_t i = 0; i < n_src; ++i) {
        fa(i) = *rc::gen::map(rc::gen::inRange(1, 10001),
                              [](int v) { return static_cast<double>(v) / 10000.0; });
        aa(i) = *rc::gen::map(rc::gen::inRange(1, 10001),
                              [](int v) { return static_cast<double>(v) / 100.0; });
    }
    for (std::size_t j = 0; j < n_dst; ++j) {
        fb(j) = *rc::gen::map(rc::gen::inRange(1, 10001),
                              [](int v) { return static_cast<double>(v) / 10000.0; });
        ab(j) = *rc::gen::map(rc::gen::inRange(1, 10001),
                              [](int v) { return static_cast<double>(v) / 100.0; });
    }

    // Build the original InterpolationMatrix
    axis::solver::InterpolationMatrix<Kokkos::HostSpace> original(
    ...
    const std::size_t blob_size =
        axis::solver::WeightCache::serialize(original, nullptr, 0);
    RC_ASSERT(blob_size > 0);

    std::vector<uint8_t> blob(blob_size);
    const std::size_t written =
        axis::solver::WeightCache::serialize(original, blob.data(), blob_size);
    RC_ASSERT(written == blob_size);

    // Deserialize
    auto restored =
        axis::solver::WeightCache::deserialize<Kokkos::HostSpace>(
            blob.data(), blob_size);
    ...
    RC_ASSERT(std::memcmp(dst_original.data(), dst_restored.data(),
                          n_dst * sizeof(double)) == 0);
}
