...
void validateCsrInvariants(
    const axis::topology::UnstructuredMesh<Kokkos::HostSpace>& mesh)
{
...
RC_GTEST_PROP(CsrConnectivityProperty3, FromStructuredGrid, ()) {
    ...
    Kokkos::View<double*, Kokkos::HostSpace> center_lon(
        "center_lon", n_centers);
    Kokkos::View<double*, Kokkos::HostSpace> center_lat(
        "center_lat", n_centers);
...
RC_GTEST_PROP(CsrConnectivityProperty3, DirectlyConstructedMesh, ()) {
    ...
    const std::size_t verts_per_cell = *rc::gen::element(
        std::size_t{3}, std::size_t{4});
    const std::size_t nnz = n_cells * verts_per_cell;
    // Ensure enough nodes for all indices to be valid
    const std::size_t n_nodes = *rc::gen::inRange<std::size_t>(
        verts_per_cell, std::max(verts_per_cell, n_cells * verts_per_cell) + 1);

    // Build CSR offsets (uniform cells: each cell has verts_per_cell nodes)
    Kokkos::View<axis::index_t*, Kokkos::HostSpace> conn_offsets(
        "conn_offsets", n_cells + 1);
    for (std::size_t c = 0; c <= n_cells; ++c) {
        conn_offsets(c) = static_cast<axis::index_t>(c * verts_per_cell);
    }

    // Build CSR indices with random valid node indices in [0, n_nodes)
    Kokkos::View<axis::index_t*, Kokkos::HostSpace> conn_indices(
        "conn_indices", nnz);
    for (std::size_t k = 0; k < nnz; ++k) {
    ...
    Kokkos::View<double**, Kokkos::LayoutLeft, Kokkos::HostSpace> node_coords(
        "node_coords", n_nodes, std::size_t{2});
    ...
    axis::topology::UnstructuredMesh<Kokkos::HostSpace> mesh(
...
RC_GTEST_PROP(CsrConnectivityProperty3, MixedElementMesh, ()) {
    ...
    const std::size_t n_nodes = *rc::gen::inRange<std::size_t>(
        std::max(std::size_t{3}, *std::max_element(
            verts_per_cell.begin(), verts_per_cell.end())),
        total_verts + 10);
    ...
    conn_offsets(0) = 0;
    for (std::size_t c = 0; c < n_cells; ++c) {
        conn_offsets(c + 1) = conn_offsets(c) +
                              static_cast<axis::index_t>(verts_per_cell[c]);
    }

    // Build CSR indices with random valid node indices
    Kokkos::View<axis::index_t*, Kokkos::HostSpace> conn_indices(
        "conn_indices", total_verts);
