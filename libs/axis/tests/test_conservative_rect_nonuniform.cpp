...
/// @file tests/test_conservative_rect_nonuniform.cpp
/// @brief GTest unit tests for the conservative non-uniform rectilinear grid fast-path.
...
TEST(ConservativeRectNonUniform, KnownAnalyticOverlaps) {
    ...
    // Total cells: src = 3*2 = 6, dst = 2*1 = 2
    EXPECT_EQ(matrix.n_src(), 6);
    EXPECT_EQ(matrix.n_dst(), 2);

    // Verify row sums are consistent with coverage
