...
maxnv = int(nec.max())
lb = np.zeros((n, maxnv)); ln = np.zeros((n, maxnv))
for c in range(n):
    m = int(nec[c]); vids = voc[c, :m] - 1
    lb[c, :m] = lat_v[vids]; ln[c, :m] = lon_v[vids]
    lb[c, m:] = lb[c, m - 1]; ln[c, m:] = ln[c, m - 1]
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    p = subprocess.run(["cdo", "-s", "-f", "nc4", f"remapcon,{dst_nc}", src_nc, cdo_out],
                       capture_output=True)
    assert p.returncode == 0, p.stderr[-400:]
    cdo_v = xr.open_dataset(cdo_out)["temperature"].values
    print(f"\nCDO remapcon        : sum={np.nansum(cdo_v):+.4f}  "
          f"nan={int(np.isnan(cdo_v).sum())}/{cdo_v.size}")
    ...
    for lt in ("great_circle", "cartesian"):
        r = axis.Regridder(ds_in, ds_out, method="conservative", periodic=True,
                           norm_type="fracarea", line_type=lt)
        v = r(ds_in["temperature"]).values
        print(f"AXIS {lt:<13} : sum={np.nansum(v):+.4f}  "
              f"nan={int(np.isnan(v).sum())}/{v.size}  "
