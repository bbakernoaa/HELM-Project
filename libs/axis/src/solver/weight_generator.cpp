...
InterpolationMatrix<MemorySpace> generate_budget(const topology::UnstructuredMesh<MemorySpace> &src_mesh,
    ...
    // Access destination coords and connectivity (mirrored to HostSpace to avoid host-device space dereference hazards)
    auto dst_coords = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, dst_mesh.node_coords_view());
    auto dst_off = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, dst_mesh.conn_offsets_view());
    auto dst_idx = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, dst_mesh.conn_indices_view());
