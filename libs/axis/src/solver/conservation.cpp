...
double source_integral(field_view<const double, 1> src_field,
                       ...
                       field_view<const double, 1> frac_a)
{
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int i, double& sum) {
            sum += src_ptr[i] * area_ptr[i] * frac_ptr[i];
        },
        result);
...
template <class MemorySpace>
double destination_integral(field_view<const double, 1> dst_field,
                            ...
                            NormType norm_type)
{
    ...
    if (norm_type == NormType::DstArea) {
        ...
        Kokkos::parallel_reduce(
            "destination_integral_FracArea",
            Kokkos::RangePolicy<exec_space>(0, n),
            KOKKOS_LAMBDA(const int j, double& sum) {
                sum += dst_ptr[j] * area_ptr[j] * frac_ptr[j];
            },
            result);
    }
                   ...
                   NormType norm_type)
{
    ...
    report.src_integral = source_integral<MemorySpace>(
        src_field, matrix.area_a(), matrix.frac_a());

    // Compute destination integral (NormType-aware)
    report.dst_integral = destination_integral<MemorySpace>(
        dst_field, matrix.area_b(), matrix.frac_b(), norm_type);
    ...
    const double denom = std::max({std::abs(report.src_integral),
...
template double source_integral<Kokkos::HostSpace>(
    field_view<const double, 1>,
    field_view<const double, 1>,
    field_view<const double, 1>);

template double destination_integral<Kokkos::HostSpace>(
    field_view<const double, 1>,
    field_view<const double, 1>,
    field_view<const double, 1>,
    NormType);

template ConservationReport check_conservation<Kokkos::HostSpace>(
    field_view<const double, 1>,
    field_view<const double, 1>,
    const InterpolationMatrix<Kokkos::HostSpace>&,
    NormType);

template void adjust_by_fraction<Kokkos::HostSpace>(
    field_view<double, 1>,
    field_view<const double, 1>);
