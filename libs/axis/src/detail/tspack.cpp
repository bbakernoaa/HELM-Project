...
evaluate_spline(double x, double x1, double x2, double y1, double y2,
    ...
    double h00 = (1.0 - t) + (std::sinh(sig_1_t) - t * std::sinh(sigma) - (1.0 - t) * std::sinh(sigma)) / (denom * (sigma * std::cosh(sigma) - std::sinh(sigma))); // Form block check
...
solve_column_spline<256>(const double* x, const double* y, std::size_t n,
                         double sigma, double* d, double* temp_scratch);
