...
const char* convention_kind_str(ingest::ConventionKind kind) {
    switch (kind) {
        case ingest::ConventionKind::CF:        return "CF";
        case ingest::ConventionKind::UGRID:     return "UGRID";
        case ingest::ConventionKind::GRIB:      return "GRIB";
        case ingest::ConventionKind::Projected: return "Projected";
        case ingest::ConventionKind::NamedGrid: return "NamedGrid";
        case ingest::ConventionKind::GridRules: return "GridRules";
    }
    return "Unknown";
}
...
void validate_cf(const ingest::GridDescriptor& desc) {
    ...
    if (b.center_x.data_handle() == nullptr || b.center_x.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(CF): null/empty required buffer 'center_x'");
    }
    if (b.center_y.data_handle() == nullptr || b.center_y.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(CF): null/empty required buffer 'center_y'");
    }

    if (b.center_x.extent(0) != expected) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(CF): inconsistent buffer extents — "
            "center_x.extent(0)=" + std::to_string(b.center_x.extent(0)) +
            " but ni*nj=" + std::to_string(expected));
    }
    if (b.center_y.extent(0) != expected) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(CF): inconsistent buffer extents — "
            "center_y.extent(0)=" + std::to_string(b.center_y.extent(0)) +
            " but ni*nj=" + std::to_string(expected));
    }
}
...
void validate_ugrid(const ingest::GridDescriptor& desc) {
    ...
    if (b.node_coords.data_handle() == nullptr || b.node_coords.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(UGRID): null/empty required buffer 'node_coords'");
    }
    if (b.conn_offsets.data_handle() == nullptr || b.conn_offsets.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(UGRID): null/empty required buffer 'conn_offsets'");
    }
    if (b.conn_indices.data_handle() == nullptr || b.conn_indices.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(UGRID): null/empty required buffer 'conn_indices'");
    }
    ...
    if (b.node_coords.extent(1) < 2) {
        throw std::invalid_argument(
            ...
            "node_coords.extent(1)=" + std::to_string(b.node_coords.extent(1)) +
            " but ndim must be >= 2");
    }
}
...
void validate_grib(const ingest::GridDescriptor& desc) {
    ...
    if (g.grid_type.empty()) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(GRIB): missing required field 'grid_type'");
    }
    if (g.ni <= 0) {
        throw std::invalid_argument(
    ...
    const std::size_t expected = static_cast<std::size_t>(g.ni) *
                                 static_cast<std::size_t>(g.nj);

    if (b.center_x.data_handle() == nullptr || b.center_x.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(GRIB): null/empty required buffer 'center_x'");
    }
    if (b.center_y.data_handle() == nullptr || b.center_y.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(GRIB): null/empty required buffer 'center_y'");
    }
...
void validate_projected(const ingest::GridDescriptor& desc) {
    ...
    if (b.center_x.data_handle() == nullptr || b.center_x.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(Projected): null/empty required buffer 'center_x'");
    }
    if (b.center_y.data_handle() == nullptr || b.center_y.extent(0) == 0) {
        throw std::invalid_argument(
            "MeshFactory::from_descriptor(Projected): null/empty required buffer 'center_y'");
    }
...
copy_buffer_1d(const char* label, field_view<const T, 1> src) {
...
copy_buffer_2d(const char* label, field_view<const T, 2> src) {
    ...
    Kokkos::View<const T**, Kokkos::LayoutLeft, Kokkos::HostSpace,
                 Kokkos::MemoryTraits<Kokkos::Unmanaged>>
        src_view(src.data_handle(), n0, n1);
...
template <class MemorySpace, class T>
Kokkos::View<T*, MemorySpace>
adopt_or_copy_1d(const char* label, field_view<const T, 1> src) {
    return copy_buffer_1d<MemorySpace, T>(label, src);
}

template <class MemorySpace, class T>
Kokkos::View<T**, Kokkos::LayoutLeft, MemorySpace>
adopt_or_copy_2d(const char* label, field_view<const T, 2> src) {
...
CoordinateSystem map_coord_system(ingest::CoordinateSystem cs) {
    switch (cs) {
        case ingest::CoordinateSystem::SphericalDeg: return CoordinateSystem::SphericalDeg;
        case ingest::CoordinateSystem::SphericalRad: return CoordinateSystem::SphericalRad;
        case ingest::CoordinateSystem::Cartesian3D:  return CoordinateSystem::Cartesian3D;
    }
...
build_from_cf(const ingest::GridDescriptor& desc) {
    const auto& b = desc.buffers;
    ...
    StructuredGrid<MemorySpace> grid(
        b.ni, b.nj,
        std::move(center_lon),
        std::move(center_lat),
        map_coord_system(desc.coord_system));

    // If corners are provided, set them.
    if (b.corner_x.data_handle() != nullptr && b.corner_x.extent(0) > 0 &&
        b.corner_y.data_handle() != nullptr && b.corner_y.extent(0) > 0) {
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
build_from_ugrid(const ingest::GridDescriptor& desc) {
    const auto& b = desc.buffers;

    // Copy node coordinates [n_nodes, ndim].
    auto node_coords = adopt_or_copy_2d<MemorySpace, double>(
        "ugrid_node_coords", b.node_coords);

    // Copy CSR connectivity.
    auto conn_offsets = adopt_or_copy_1d<MemorySpace, index_t>(
        "ugrid_conn_offsets", b.conn_offsets);
    auto conn_indices = adopt_or_copy_1d<MemorySpace, index_t>(
        "ugrid_conn_indices", b.conn_indices);
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
build_from_grib(const ingest::GridDescriptor& desc) {
    const auto& g = desc.grib;
    const auto& b = desc.buffers;
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
build_from_projected(const ingest::GridDescriptor& desc) {
    auto grid = ProjectionBuilder::build<MemorySpace>(
        desc.projected, desc.buffers);
    return grid.to_unstructured();
}
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
build_from_named_grid(const ingest::GridDescriptor& desc) {
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
build_from_grid_rules(const ingest::GridDescriptor& desc) {
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
MeshFactory::from_descriptor(const ingest::GridDescriptor& descriptor) {
    ...
    throw std::invalid_argument(
        "MeshFactory::from_descriptor: unknown ConventionKind value (" +
        std::to_string(static_cast<int>(descriptor.kind)) + ")");
}
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
MeshFactory::from_named(const std::string& name) {
...
template <class MemorySpace>
UnstructuredMesh<MemorySpace>
MeshFactory::from_rules(const ingest::GridRulesParams& params) {
...
MeshFactory::from_descriptor<Kokkos::HostSpace>(const ingest::GridDescriptor&);

template UnstructuredMesh<Kokkos::HostSpace>
MeshFactory::from_named<Kokkos::HostSpace>(const std::string&);

template UnstructuredMesh<Kokkos::HostSpace>
MeshFactory::from_rules<Kokkos::HostSpace>(const ingest::GridRulesParams&);

// CudaSpace (when CUDA is enabled)
#ifdef KOKKOS_ENABLE_CUDA
template UnstructuredMesh<Kokkos::CudaSpace>
MeshFactory::from_descriptor<Kokkos::CudaSpace>(const ingest::GridDescriptor&);

template UnstructuredMesh<Kokkos::CudaSpace>
MeshFactory::from_named<Kokkos::CudaSpace>(const std::string&);

template UnstructuredMesh<Kokkos::CudaSpace>
MeshFactory::from_rules<Kokkos::CudaSpace>(const ingest::GridRulesParams&);
#endif

// HIPSpace (when HIP is enabled)
#ifdef KOKKOS_ENABLE_HIP
template UnstructuredMesh<Kokkos::HIPSpace>
MeshFactory::from_descriptor<Kokkos::HIPSpace>(const ingest::GridDescriptor&);

template UnstructuredMesh<Kokkos::HIPSpace>
MeshFactory::from_named<Kokkos::HIPSpace>(const std::string&);

template UnstructuredMesh<Kokkos::HIPSpace>
MeshFactory::from_rules<Kokkos::HIPSpace>(const ingest::GridRulesParams&);
#endif
